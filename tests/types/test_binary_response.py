...
class BinaryResponseUnitTest(unittest.TestCase):
    ...
    def test_mime_type(self):
        ...
        self.assertEqual(binary_response.mime_type, mime_type_a)

        mime_type_b = "image/png"
        binary_response = BinaryResponse()
        self.assertIsNone(binary_response.mime_type)
        binary_response.mime_type = mime_type_b
        self.assertEqual(binary_response.mime_type, mime_type_b)
    ...
    def test_file_name(self):
        ...
        self.assertEqual(binary_response.file_name, file_name_a)

        file_name_b = "file.txt"
        binary_response = BinaryResponse()
        self.assertIsNone(binary_response.file_name)
        binary_response.file_name = file_name_b
        self.assertEqual(binary_response.file_name, file_name_b)
    ...
    def test_contents(self):
        ...
        self.assertEqual(binary_response.contents, contents)

    def test_content_length(self):
        binary_response = BinaryResponse()
        self.assertEqual(binary_response.content_length, 0)

        contents = "contents of binary response"
        binary_response.contents = contents
        self.assertEqual(binary_response.contents, contents)
        self.assertEqual(binary_response.content_length, len(contents))
